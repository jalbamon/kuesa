...
private Q_SLOTS:
    ...
    void checkBoxCompression()
    {
        ...
        // THEN
        {
            ...
            ctx = GLTF2Context{};
            auto res = parser.parseSynchronous(QJsonDocument{ exported.json() }.toJson(), tmp.absolutePath(), QStringLiteral("test.gltf"));
            QVERIFY(res != nullptr);
    ...
    void compressedBufferFilenameIncrement()
    {
        ...
        // THEN
        {
            ...
            ctx = GLTF2Context{};
            auto res = parser.parseSynchronous(QJsonDocument{ exported.json() }.toJson(), sub.absolutePath(), QStringLiteral("test.gltf"));
            QVERIFY(res != nullptr);
        }
    }
#endif
